...
In general, the findings of this code review indicate that the BitWarden Desktop Application maintains a solid basis for security assurance. From the robust and secure source code to the clean formatting and coding practices, few weaknesses were discovered while canvassing the code. Noted weaknesses, while not perfectly mitigated, are handled responsibly and to the largest degree effectively. Minor issues in input validation, obscure encryption packages, and a sparse smattering of clumsy code are the only potential weaknesses exhibited by the source code. As such, assurance is high for the security of this application and risk is low for any business, group, or individual that chooses to implement this application.
...
Our approach to this portion of the assignment started off as with a lot of discussion and interactions between one another. Before meeting with professor Gandhi, we wanted to be sure we at least had some initial results that could be presented and discussed. We used a cloud-based automated review tool for our first scan set. Potential problems and vulnerabilities were discussed and distributed among the group. Communication was really fluid and each team member was encouraged to share scan results from their own automated scans. Manual code review questions and concerns were quickly addressed by fellow group members and resolved. Compared to other phases in the project, this phase was very organized, especially when contrasted with the first phase of the project. As a group we've all grown and adapted to the responsibilities and duties we found ourselves within. Overall, this has been a fantastic team with no noted significant issues arising either personally or organizationally through the semester.
